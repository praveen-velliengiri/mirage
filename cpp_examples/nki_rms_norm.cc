...
int main(int argc, char **argv) {
  ...
  std::string suffix =
      std::to_string(M) + "_" + std::to_string(K) + "_" + std::to_string(N);
  std::string checkpoint_filename =
      "nki_rms_norm_checkpoint_" + suffix + ".json";
  if (generate_programs_from_checkpoint) {
    std::ifstream generated_graphs_file(checkpoint_filename.data(),
                                        std::ifstream::binary);
    if (!generated_graphs_file) {
      std::cerr << "Error: Could not open file " << checkpoint_filename
                << std::endl;
      return 1;
    }
    ...
    for (json const &graph_json : j) {
      ...
      from_json(graph_json, graph);
      {
        ...
        nki_transpiler::NKITranspileResult transpile_result =
            transpiler.generate_code();
        if (transpile_result.error_state.errors.empty()) {
          // save transpiled code to nki_programs/nki_program_<idx>.py
          std::string filename = "nki_programs" + suffix + "/nki_program_" +
                                 std::to_string(idx) + ".py";
          ...
          ofs.close();
          std::cout << "Transpiled NKI program saved to " << filename
                    << std::endl;
        } else {
          std::cerr << "Error in transpiling graph " << idx << ": ";
          for (auto const &error : transpile_result.error_state.errors) {
            std::cerr << error << " ";
          }
          std::cerr << std::endl;
        }
  ...
  search::KernelGraphGenerator gen(
      ref_graph, config, checkpoint_filename.data());
